...
if [[ "$CONFIGURE_DNS" == "y" ]]; then
    #read -p "Enter your DNS Server IPs (Single Host only): " DNS_SERVERS
...
if [[ "$INSTALL_RISKIQ" == "y" ]]; then
    ...
    sed -i "s|EF_PROCESSOR_ENRICH_IPADDR_RISKIQ_ASN_ENABLE: 'false'|EF_PROCESSOR_ENRICH_IPADDR_RISKIQ_ASN_ENABLE: 'true'|g" docker-compose.yml
